...
var customersTableColumns = []string{"customer_id", "name", "date_of_birth", "city", "zipcode", "status"}

const dummyStatus = ""
const selectAllCustomersSql = "SELECT customer_id, name, city, zipcode, date_of_birth, status FROM customers"
const selectSpecificCustomersSql = "SELECT customer_id, name, city, zipcode, date_of_birth, status FROM customers WHERE status = ?"
const selectCustomersSql = "SELECT customer_id, name, city, zipcode, date_of_birth, status FROM customers WHERE customer_id = ?"
...
func TestCustomerRepositoryDb_FindAll_returns_all_customers_when_selectCustomers_succeed_and_status_empty(t *testing.T) {
	...
	for _, v := range dummyCustomers {
		dummyRows.AddRow(v.Id, v.Name, v.DateOfBirth, v.City, v.Zipcode, v.Status)
	}
...
func TestCustomerRepositoryDb_FindAll_returns_specific_customers_when_selectCustomers_succeed_and_status_nonEmpty(t *testing.T) {
	...
	dummyRows := sqlmock.NewRows(customersTableColumns).
		AddRow(dummyActiveCustomer.Id, dummyActiveCustomer.Name, dummyActiveCustomer.DateOfBirth, dummyActiveCustomer.City, dummyActiveCustomer.Zipcode, dummyActiveCustomer.Status)
...
func TestCustomerRepositoryDb_FindById_returns_customer_when_selectCustomers_succeeds(t *testing.T) {
	...
	dummyRows := sqlmock.NewRows(customersTableColumns).
		AddRow(dummyCustomer.Id, dummyCustomer.Name, dummyCustomer.DateOfBirth, dummyCustomer.City, dummyCustomer.Zipcode, dummyCustomer.Status)
	mockDB.ExpectQuery(selectCustomersSql).
